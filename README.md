...
### 4. 유사도 기법 선택
* 코사인 유사도, 피어슨 유사도, 유클리드 유사도, 자카드 유사도를 비교하였다.
  * 자카드 유사도는 임베딩을 이용한 것이 아니라 토큰화만 되어 있으면 되었기 때문에, 각 형태소 분석기 별로 자카드 유사도를 이용한 결과를 비교했고, 역시 높은 성능을 보였던 kkma로 최종 선택했다.
...
## RAG 기반
### 1. 모델
* 학습: "EleutherAI/polyglot-ko-1.3b" 사용, 1에포크 학습
  * 토크나이저도 동일 토크나이저 사용
* 검색에 사용할 모델과 토크나이저
  * 의미 검색의 임베딩 모델은 "snunlp/KR-SBERT-V40K-klueNLI-augSTS", 키워드 검색의 토크나이저는 "klue/roberta-base" 사용
### 2. 학습 및 추론
...
* validation
  ...
  * check_chatbot_result.ipynb: 역번역 질문을 이용하여 정답을 맞힌 개수를 확인. 각 단계 별로 후보들을 비교
* similarity_based_chatbot_vs1.ipynb: 최종적으로 선택된 것들을 이용하여 질의응답을 완성하고 결과를 확인. (법률구조공단의 데이터 포함)
* web
  * modules.py: 위 챗봇 코드를 정리하여 모듈을 모아놓은 것
  * forms.py: 프론트엔드(react)와 데이터를 주고 받는 코드
  * pan_recommand.js: 메인이 되는 페이지. 사용자 입력을 받아 서버에 보내고 추천 결과를 받아 출력
  * loading_modal.js: 로딩 중일 때 보여줄 페이지
### RAG based
